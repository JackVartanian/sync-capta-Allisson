    ...
    SELECT
        ...
        WHEN RTRIM( a.emps ) LIKE 'CUR' THEN 'BAT' --filtra empresa
    ...
    WHERE a.codbarras <> 0
        ...
        AND A.DOPES NOT IN ('ADTO ENCOMENDA', 'VENDA PERMUTA', 'VENDA FUNCIONARIO', 'VENDA ENCOMENDA', 'VENDA ENC E-COMM')
        -- AND RTRIM( a.iclis ) = 'CFLO000296'
UNION ALL
    SELECT
        CASE
                WHEN RTRIM( a.emps ) LIKE 'IGL' THEN 'IGU'
...
UNION ALL
    ...
    WHERE EEST.emps <> 'NY'
    -- AND RTRIM( EEST.DOPES) IN ('ADTO ENCOMENDA', 'ENCOMENDA E-COMM', 'VENDA E-COMMERCE', 'VENDA', 'VENDA FR', 
    --     'VENDA CONSIGNACAO', 'TROCA')
        AND EEST.DOPES IN ('ADTO ENCOMENDA ', 'ENCOMENDA E-COMM')
        AND EESTI.unitorigs <> 0
        AND EEST.dopes NOT IN ('VENDA PERMUTA', 'VENDA FUNCIONARIO', 'VENDA ENCOMENDA', 'VENDA ENC E-COMM')
        -- AND RTRIM( EEST.Contads ) = 'CFLO000296'
...
UNION ALL
    ...
    FROM SLJEEST AS EEST with(nolock)
        ...
        LEFT JOIN sljeti AS M with(nolock) ON EESTI.codbarras = M.cbars
    WHERE 
        RTRIM( EEST.DOPES) IN ('ADTO ENCOMENDA', 'ENCOMENDA E-COMM', 'VENDA E-COMMERCE','VENDA ENCOM SINAL', 'VENDA', 'VENDA FR', 
        'VENDA CONSIGNACAO', 'TROCA')
...
--        LEFT JOIN SLJCLI AS K with(nolock) ON K.ICLIS = EEST.VENDS
--        LEFT JOIN sljeti AS M with(nolock) ON EESTI.codbarras = M.cbars
--    WHERE RTRIM( EEST.DOPES) NOT IN ('VENDA PERMUTA', 'VENDA FUNCIONARIO', 'VENDA ENCOMENDA', 'VENDA ENC E-COMM')
--    AND RTRIM( EEST.DOPES) IN ('ADTO ENCOMENDA', 'ENCOMENDA E-COMM', 'VENDA E-COMMERCE', 'VENDA', 'VENDA FR', 'VENDA CONSIGNACAO', 'TROCA')
--    -- AND EEST.emps = 'IGL'
--    AND EESTI.unitorigs <> 0
--    -- AND RTRIM( EEST.Contads ) = 'CFLO000296'
--    AND EEST.datas >= CONCAT( CONVERT( date, GETDATE()), ' 00:00:00')
