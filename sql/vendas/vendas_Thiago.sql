...
WHERE RTRIM( EEST.emps ) <> 'NY'
    AND EESTI.unitorigs <> 0
    AND CAST(EEST.datas AS DATE) < CAST(GETDATE() AS DATE)
    AND CAST(EEST.datas AS DATE) >= '2024-08-26'
    AND RTRIM( EEST.DOPES) NOT IN ('VENDA PERMUTA', 'VENDA FUNCIONARIO', 'VENDA ENCOMENDA', 'VENDA ENC E-COMM')
    AND RTRIM( EEST.DOPES) IN ('VENDA E-COMMERCE', 'VENDA', 'VENDA FR', 'VENDA CONSIGNACAO', 'TROCA', 'TROCA COM CREDITO', 'DEVOLUÇÃO DE VENDA', 'VENDA ENCOM SINAL', 'TROCA SEM BARRA', 'VENDA E-COMMERCE LJ', 'DEVOLUCAO DE VENDA', 'VENDA SF', 'DEV VENDA SF')
UNION ALL
SELECT
    CASE 
        WHEN RTRIM( EEST.emps ) LIKE 'IGL' THEN 'IGU'
