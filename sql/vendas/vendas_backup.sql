    ...
    SELECT
        ...
        CONCAT( RTRIM( a.iclis ),'-', CAST( a.datas AS DATE ), '-', CAST (a.numes AS INTEGER ),'-', RTRIM( a.cpros ), '-', CAST( a.totas AS DECIMAL(10)), '-', RTRIM( a.codbarras )) AS ID_Drop,
...
--     WHERE RTRIM( a.emps ) <> 'NY'
        AND a.vvistas <> 0
        AND a.tipoops NOT IN (91, 92)
        AND RTRIM( a.dopes ) NOT IN ('VENDA PERMUTA', 'VENDA FUNCIONARIO', 'VENDA ENCOMENDA', 'VENDA ENC E-COMM', 'ADTO ENCOMENDA', 'DEV CRED ENCOMENDA', 'DEV VENDA SF', 'DEVOL VENDA EXT', 'DEVOLUCAO DE VENDA', 'ENCOMENDA E-COMM', 'RECALL PERDA', 'TROCA', 'TROCA COM CREDITO', 'TROCA SEM BARRA', 'VENDA', 'VENDA ANIVERSARIANTE', 'VENDA CONSIG SF', 'VENDA CONSIGNACAO', 'VENDA E-COMMERCE', 'VENDA E-COMMERCE LJ', 'VENDA ENCOM SINAL', 'VENDA EXT', 'VENDA FINAL', 'VENDA FR', 'VENDA LJ EXTERIOR', 'VENDA SF', 'VENDA US')
        -- AND RTRIM( a.dopes ) NOT IN ('ADTO ENCOMENDA', 'VENDA PERMUTA', 'VENDA FUNCIONARIO', 'VENDA ENCOMENDA', 'VENDA ENC E-COMM')
        AND a.datas >= '2024-01-01 00:00:00'
UNION ALL
    SELECT
        ...
        CONCAT( RTRIM( EEST.Contads ),'-', CAST( EEST.datas AS DATE ), '-', CAST (EEST.numes AS INTEGER ),'-', RTRIM( EESTI.cpros ), '-', CAST( EESTI.totas + eesti.valrats AS DECIMAL(10)) * -1, '-', RTRIM( EESTI.codbarras )) AS ID_Drop,
